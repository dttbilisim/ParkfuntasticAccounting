CREATE OR REPLACE PROCEDURE "public"."sync_products_from_basbugs"()
AS $BODY$
DECLARE
    seller_id INTEGER := 2; -- Basbug SellerId
...
BEGIN
    ...
    RAISE NOTICE '═══════════════════════════════════════════════════════════';
    RAISE NOTICE '🚀 TERA-NITRO SYNC: Basbugs (Bulletproof)';
    RAISE NOTICE '   Start Time: %', v_start_time;
    RAISE NOTICE '═══════════════════════════════════════════════════════════';
    
    -- 1. HEAVY LIFTING: DISABLE TRIGGERS & INDEXES
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '⚠️ Warning: Some optimizations could not be applied.';
    END;
    ...
    SELECT
        pb."Id"::TEXT AS "SourceId",
        INITCAP(TRIM(COALESCE(pb."Aciklama1", '') || ' ' || COALESCE(pb."Aciklama2", '') || ' ' || COALESCE(pb."Model", ''))) AS "ProductName",
        TRIM(pb."Uretici") AS "BrandName",
        UPPER(TRIM(COALESCE(pb."OemKod", ''))) || '|' || UPPER(TRIM(pb."No")) AS "NormalizedOem",  
        pb."Fiyat" AS "Price",
        CASE 
            WHEN pb."ParaBirimi" = 'TL' THEN 'TRY'
            WHEN pb."ParaBirimi" = 'USD' THEN 'USD'
            WHEN pb."ParaBirimi" = 'EUR' THEN 'EUR'
            ELSE 'TRY'
        END AS "Currency",
        COALESCE(pb."Stok", 0) AS "Stock",
        1 as "CreatedId", 
        NOW() as "CreatedDate"
    FROM "ProductBasbugs" pb
    WHERE pb."Fiyat" IS NOT NULL AND TRIM(pb."OemKod") IS NOT NULL;
    ...
    SELECT DISTINCT ON (TRIM("BrandName")) INITCAP(TRIM("BrandName")), 1, 1, NOW(), 1
    FROM tmp_incoming WHERE "BrandName" IS NOT NULL
    ...
    SELECT 
        product_name, 'TMP-BASBUGS-' || source_id, min_amount, 1, 1, 1, 1,
    ...
    SET product_id = p."Id",
        is_new = TRUE
    FROM "Product" p
    WHERE p."Barcode" = 'TMP-BASBUGS-' || tm.source_id 
      AND tm.product_id IS NULL;

    -- Clear Temporary Barcodes
    UPDATE "Product" SET "Barcode" = NULL WHERE "Barcode" LIKE 'TMP-BASBUGS-%';
    ...
    RAISE NOTICE '🏁 TERA-NITRO BASBUGS COMPLETED. Products: %, OEMs: %, Items: %, Time: %s', 
