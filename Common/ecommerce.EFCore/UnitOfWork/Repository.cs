...
public sealed class Repository<TEntity> : IRepository<TEntity> where TEntity : class
{
    ...
    public IQueryable<TResult> GetAll<TResult>(
        ...
        disableTracking
            ...
            : _dbSet.AsTracking().Select(selector);
    ...
    // helper: detach any locally tracked entity with same primary key
    public void DetachLocalByKey(TEntity entity)
    {
        var typeInfo = typeof(TEntity).GetTypeInfo();
        var entityType = _dbContext.Model.FindEntityType(typeInfo);
        var keyProp = entityType?.FindPrimaryKey()?.Properties.FirstOrDefault();
        if (keyProp == null)
        {
            return;
        }
        var prop = typeInfo.GetProperty(keyProp.Name);
        if (prop == null)
        {
            return;
        }
        var keyVal = prop.GetValue(entity);
        if (keyVal == null)
        {
            return;
        }
        var local = _dbSet.Local.FirstOrDefault(e => Equals(prop.GetValue(e), keyVal));
        if (local != null)
        {
    ...
    // helper: attach entity as Modified, optionally prevents navigation updates
    public void AttachAsModified(TEntity entity, bool excludeNavigations = true)
    {
        DetachLocalByKey(entity);
