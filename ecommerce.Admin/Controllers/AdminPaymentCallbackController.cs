...
public class AdminPaymentCallbackController : Controller
{
    ...
    public AdminPaymentCallbackController(
    ...
    public async Task<IActionResult> Callback(IFormCollection form)
    {
        ...
        try 
        {
            ...
            var sbEarly = new System.Text.StringBuilder();
            sbEarly.AppendLine($"\n--- CALLBACK ENTERED ADMIN ({DateTime.Now}) ---");
            sbEarly.AppendLine($"User IsAuth: {User.Identity?.IsAuthenticated}");
            sbEarly.AppendLine($"User ID: {User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value}");
            sbEarly.AppendLine($"Query Token: {Request.Query["token"]}");
            sbEarly.AppendLine($"Form OID: {form["oid"]}");
            sbEarly.AppendLine($"Form mdStatus: {form["mdStatus"]}");
            sbEarly.AppendLine($"Form Response: {form["Response"]}");
            sbEarly.AppendLine("--------------------------\n");
            System.IO.File.AppendAllText(debugPath, sbEarly.ToString());
        } catch {}
        ...
        try
        {
            ...
            if (orders == null || !orders.Any())
            {
                 ...
                 {
                     var debugPath = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "payment_debug.txt");
                     System.IO.File.AppendAllText(debugPath, $"Order Search FAILED. Target Token: {paymentToken ?? "NULL"}. Redis: {cacheKey ?? "NULL"}\n");
                 } catch {}

                 // Only return error if BOTH methods failed
                 return ReturnModalResponse(false, "Sipariş bulunamadı veya ödeme oturumu sona erdi. Lütfen sipariş geçmişinizi kontrol ediniz.");
            }

            var firstOrder = orders.First();
            if (firstOrder.Bank == null || !Enum.TryParse(firstOrder.Bank.SystemName, out ecommerce.Core.Utils.BankNames bankNameEnum))
            {
                return ReturnModalResponse(false, "Banka bilgisi geçersiz");
            }
            ...
            try 
            {
                ...
                sbRaw.AppendLine($"\n--- RAW BANK CALLBACK ADMIN ({DateTime.Now}) ---");
            ...
            // Admin B2B uses admin.yedeksen.com
            var baseUrl = _configuration["AppSettings:AdminBaseUrl"] ?? "https://admin.yedeksen.com";
            var constructedCallbackUrl = $"{baseUrl}/admin-checkout/callback?token={paymentToken}";
            ...
            try 
            {
                ...
                sbLog.AppendLine($"\n--- CALLBACK RECEIVED ADMIN ({DateTime.Now}) ---");
            ...
            if (result.Success)
            {
                ...
                // CLEAR USER CART using OrderManager (handles both Redis and DB)
                if (int.TryParse(userIdStr, out var uId))
                {
                    ...
                    {
                         // Fail silently but log internal error
                         Console.WriteLine($"Cart clearing error in callback: {ex.Message}");
                    }
                }
