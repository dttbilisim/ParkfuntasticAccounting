...
# === DB Ayarları ===
# Lutfen bu bilgileri kontrol ediniz
DB_PARAMS = dict(
    host="localhost",
    port=5454,
...
STOP_WORDS = {
    # Bağlaçlar, Edatlar, Zamirler
    've', 'ile', 'veya', 'icin', 'bir', 'bu', 'su', 'o', 'ne', 'kim', 'her', 'tum', 'bazem',
    'gibi', 'kadar', 'diye', 'ancak', 'lakin', 'fakat', 'ama', 'ise', 'ki', 'de', 'da',
    'mi', 'mu', 'mu', 'mu', 'im', 'in', 'un', 'un', 'yi', 'yu', 'dan', 'den', 'tan', 'ten',

    # Genel Nitelikler ve Yönler
    'takim', 'adet', 'set', 'komple', 'sag', 'sol', 'on', 'arka', 'ust', 'alt', 'yan', 
    'orta', 'merkez', 'ic', 'dis', 'taraf', 'yon', 'kisim', 'parca', 'bolum',
    'buyuk', 'kucuk', 'ince', 'kalin', 'dar', 'genis', 'yuksek', 'alcak', 'uzun', 'kisa',
    'yumusak', 'sert', 'hafif', 'agir', 'yeni', 'eski', 'sabit', 'seyyar',

    # Malzeme / Renk / Tip
    'metal', 'plastik', 'krom', 'celik', 'aluminyum', 'demir', 'bakir', 'pirinc',
    'siyah', 'beyaz', 'gri', 'kirmizi', 'mavi', 'sari', 'yesil', 'turuncu', 'lacivert', 
    'bordo', 'fume', 'gumus', 'altin', 'bej', 'krem', 'kahverengi', 'mor', 'pembe',
    'tip', 'model', 'cesit', 'versiyon', 'seri', 'kasasi', 'kasalari',

    # Ticari / Durum / Kaynak
    'orjinal', 'ithal', 'yerli', 'marka', 'uyumlu', 'muadil', 'yan', 'sanayi', 'esdeger',
    'fiyat', 'kampanya', 'indirim', 'ucuz', 'ekonomik', 'luks', 'kaliteli', 'garanti',
    'stok', 'kod', 'kodu', 'no', 'numara', 'referans', 'oem', 'uretim', 'imalat',

    # Otomotiv Spesifik (Gürültü Kelimeler - Kategori öğrenmeye katkısı az olanlar)
    'kapi', 'motor', 'fren', 'debriyaj', 'sanziman', 'vites', 'direksiyon', 'teker', 'jant',
    'lastik', 'cam', 'ayna', 'far', 'stop', 'sinyal', 'tampon', 'kaporta', 'amortisör',
    'benzin', 'dizel', 'lpg', 'elektrik', 'hibrit', 'araba', 'oto', 'arac', 'otomobil',
    'yedek', 'servis', 'bakim', 'onarim', 'tamir', 'revizyon', 'montaj', 'demontaj',
    'litre', 'lt', 'mm', 'cm', 'm', 'km', 'gr', 'kg', 'bar', 'psi', 'volt', 'amper', 'watt',
    'hp', 'kw', 'valf', 'sensor', 'musur', 'role', 'sigorta', 'kablo', 'hortum', 'boru',
    'civata', 'somun', 'pul', 'segman', 'conta', 'kece', 'orink', 'vida', 'percin'
}
...
def insert_category(cursor, name, parent_id=None, order=0):
    # Guncellendi: BranchId=1 kontrolu eklendi
    if parent_id:
        cursor.execute('SELECT "Id" FROM "Category" WHERE "Name" = %s AND "ParentId" = %s AND "BranchId" = 1', (name, parent_id))
    else:
        cursor.execute('SELECT "Id" FROM "Category" WHERE "Name" = %s AND "ParentId" IS NULL AND "BranchId" = 1', (name,))
        
    res = cursor.fetchone()
    if res:
        return res[0]
    
    # Guncellendi: BranchId=1 olarak insert ediliyor
    cursor.execute("""
        INSERT INTO "Category" 
        ("Name", "ParentId", "Status", "CreatedDate", "CreatedId", "IsMainPage", "IsMainSlider", "Order", "SubCategoryCount", "Height", "BranchId")
        VALUES (%s, %s, 1, NOW(), 1, FALSE, FALSE, %s, 0, 0, 1)
        RETURNING "Id"
...
def main():
    ...
    if batch_args:
        ...
        # Guncellendi: BranchId=1 eklendi
        query = 'INSERT INTO "ProductCategories" ("ProductId", "CategoryId", "BranchId") VALUES (%s, %s, 1) ON CONFLICT DO NOTHING'
        chunk_size = 5000
